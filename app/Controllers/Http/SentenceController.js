    ...
    if (!randomSentence.rows.length) {
      randomSentence = (await Database
        .raw(`
          SELECT *
          FROM sentences
          WHERE credibility_is_known_answer IS ${query.evaluated == 'true' ? 'NOT' : ''} NULL AND correct_result_clickbait IS ${query.evaluated == 'true' ? 'NOT' : ''} NULL ${ query.forbiddenIds && query.forbiddenIds.length ? `AND id NOT IN ${idsString}`: '' } 
          ORDER BY random()
          LIMIT 1;
        `)
      )
